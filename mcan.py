...
# NOTE The record based on SGA
class Merge_Att(nn.Module):
    def __init__(self, __C):
        super(Merge_Att, self).__init__()
        # self.img_sa_list = nn.ModuleList([SA(__C) for _ in range(2)])
        self.sga_list = nn.ModuleList([SAG(__C) for _ in range(2)])


    def forward(self, char, image, char_mask, image_mask):
        # for img_sa in self.img_sa_list:
        #     image = img_sa(image, image_mask)

        for sga in self.sga_list:
            char = sga(char, image, char_mask, image_mask)
